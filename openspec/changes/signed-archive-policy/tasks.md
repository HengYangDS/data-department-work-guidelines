...
- [x] 1.1 Inventory the prior archive commit, current absent policy, and
      clone-local identity; verify the tracked tree is clean and configure this
      clone from its existing effective operator identity.
- [x] 1.2 Add a regression assertion for required SSH signing without
      hardcoded author, committer, or key paths; verify it fails on the
      undeclared baseline.
- [x] 1.3 Declare the commit policy and document per-clone prerequisites;
      verify the regression and profile contract pass.

## 2. Validate and deliver the source
